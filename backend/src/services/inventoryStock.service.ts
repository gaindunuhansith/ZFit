...
export default class StockService {
    ...
    async updateStock(itemId: string, quantity: number, operation: "increment" | "decrement"): Promise<IInventoryItem | null>{
        ...
        const item = await Item.findById(itemId); // find the item id in the db 

        if(!item) throw new Error("Item not found") // if it doesn't throw an error
    ...
    async getLowStockItems(): Promise<IInventoryItem[]> {
        return await Item.find({ $expr:{ $lt: ["$quantity", "$lowStockThreshols"] } });
    }
